...
1. Obtén una clave de API (o usa LM Studio localmente):
   - **NVIDIA NIM**: <https://build.nvidia.com/settings/api-keys>
   - **OpenRouter**: <https://openrouter.ai/keys>
   - **LM Studio**: No se necesita clave. Ejecuta localmente con [LM Studio](https://lmstudio.ai)
2. Instala [Claude Code](https://github.com/anthropics/claude-code)
3. Instala [uv](https://github.com/astral-sh/uv)
4. Actualiza uv si ya está instalado: `uv self update`
...
Elige tu proveedor y edita `.env`:

<details>
<summary><b>NVIDIA NIM</b> (40 req/min gratis, recomendado)</summary>
...
<details>
<summary><b>OpenRouter</b> (cientos de modelos)</summary>
...
<details>
<summary><b>LM Studio</b> (totalmente local, sin clave de API)</summary>
...
<details>
<summary><b>Mezclar proveedores</b> (usar varios proveedores juntos)</summary>

Cada variable `MODEL_*` puede usar un proveedor distinto. `MODEL` es el fallback para modelos de Claude no reconocidos.
...
¡Listo! Claude Code ahora usa tu proveedor configurado de forma gratuita.

<details>
<summary><b>Configuración de la extensión de VSCode</b></summary>

1. Inicia el servidor proxy (igual que arriba).
2. Abre Ajustes (`Ctrl + ,`) y busca `claude-code.environmentVariables`.
3. Haz clic en **Editar en settings.json** y añade:
...
4. Recarga las extensiones.
5. **Si ves la pantalla de inicio de sesión** ("¿Cómo quieres iniciar sesión?"): Haz clic en **Consola de Anthropic**, luego autoriza. La extensión empezará a funcionar. Puede que te redirijan a comprar créditos en el navegador; ignóralo, la extensión ya funciona.

Para volver a los modelos de Anthropic, comenta el bloque añadido y recarga las extensiones.
</details>

<details>
<summary><b>Soporte multi‑modelo (selector de modèles)</b></summary>

`claude-pick` es un selector interactivo de modelos que permite elegir cualquier modelo de tu proveedor activo cada vez que lanzas Claude, sin editar `MODEL` en `.env`.

**1. Instala [fzf](https://github.com/junegunn/fzf)** (altamente recomendado para el selector interactivo):

```bash
brew install fzf        # macOS/Linux
```

**2. Añade el alias a `~/.zshrc` o `~/.bashrc`:**

```bash
# Usa la ruta absoluta a tu repositorio clonado
alias claude-pick="/ruta/absoluta/a/free-claude-code/claude-pick"
```

Luego recarga tu shell (`source ~/.zshrc` o `source ~/.bashrc`) y ejecuta `claude-pick` para elegir un modelo y lanzar Claude.

**Omite el selector con un modelo fijo** (no se necesita selector):
